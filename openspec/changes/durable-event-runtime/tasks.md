...
- [x] 4.6 Implement leased delivery claims with generation/fencing, crash recovery, stale-ack rejection, highest-contiguous-terminal checkpoints, same-subscription-stream claim ordering, and auditable terminal gaps; implement authorized requeue as a non-frontier-moving late-repair generation with out-of-order-repair capability checks, inbox-preserving ACK redelivery, and compensation/new-version fallback tests.
- [x] 4.7 Implement per-consumer batch, in-flight, and concurrency limits plus pending count, lag, and oldest-age signals; test slow consumers, backlog, storage admission failure, and recovery without unbounded memory growth.
...
- [x] 6.1 Implement `REBUILD_STATE`, `VERIFY_HISTORY`, and authorized `REDELIVER` entrypoints with separate policies and remove production use of replay-to-live-bus behavior.
- [x] 6.2 Implement pure reducer registration, ordered schema/integrity validation, checkpoint plus `after_sequence` resume, a transactionally captured finite source high watermark, source-history immutability, and durable replay reports.
- [x] 6.3 Record activity contracts for LLM, Tool, MCP, HTTP, retrieval, memory write, artifact publication, real clock, and random outcomes and make replay consume recorded references instead of invoking live operations.
- [x] 6.4 Implement workflow/reducer/policy/schema/activity version resolution and deterministic command comparison with typed mismatch, missing-activity, corrupt-history, and incompatible-version reports.
- [x] 6.5 Add failure tests for replay interruption/resume, replay concurrent with live append proving events above the captured watermark are excluded, corrupted checkpoints, unsorted input, unknown schema, upcaster failure, missing activity result, and poison-event redelivery.
...
- [x] 8.1 Add an application-owned event reader/operations service for run queries, projection export, quarantine, replay, and dead-letter actions; keep interfaces independent of concrete stores and dispatchers.
...
- [ ] 9.5 Execute the phase-specific rollback drill and prove rollback preserves accepted events and sequences, does not repeat external effects, does not disable schema/security checks, and can rebuild compatible projections. Before the compatibility observation window starts, use a pre-existing independent release-security/change-control governance bootstrap root to sign trust-activation record D, binding a positive trust epoch, exact active-policy checksum, mutually independent governance/observer/consumer-owner Ed25519 roots, a content-addressed verifier build, activation deployment/environment/time/evidence, and governance attestor; require the active policy and compiled production-verifier constants to match D on trust epoch, root identities/keys/fingerprints, Ed25519 policy, and active-policy checksum, require activation deployment <= governance signature < A window start, require independently auditable non-backfillable deployment/transparency/trusted-timestamp evidence for that time anchor, and fail closed for pending/null/mismatched trust or self-selected CLI PEM roots. Then independently retain the ordered compatibility qualification chain consisting of a signed real bounded compatibility-release deployment observation (A), a signed external consumer-owner approval of the exact qualified deletion build plan (B), deployment of only that approved build, and a signed post-deployment observer attestation binding A and B to the actual deployment (C), with A/B/C each binding D's trust epoch and record checksum, matching environments, and D retention covering C. The rollback qualification chain and bootstrap governance root -> D -> A -> B -> deploy -> C chain must both pass before task completion or deletion-release qualification, and neither substitutes for the other.
