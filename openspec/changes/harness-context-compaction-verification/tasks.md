...
- [ ] 7.1 Implement `ContextCompactionRuntime` state transitions from source snapshot through plan/apply/materialize/verify/commit-or-reject with typed outcomes.
- [ ] 7.2 Replace `ContextAssembler._compress_dynamic_tail()` string halving and unconditional verified event with the bounded runtime.
- [ ] 7.3 Preserve no-compaction behavior when source physical admission already passes and record `NO_COMPACTION_REQUIRED` without fabricated records.
- [ ] 7.4 Enforce `PROTECTED_CONTEXT_EXCEEDS_WINDOW`, `NO_ALLOWED_COMPACTION`, `ACTION_BUDGET_EXHAUSTED`, `SUMMARY_REJECTED`, and `POST_COMPACTION_VERIFY_FAILED` fail-closed paths.
- [ ] 7.5 Ensure a verified result can authorize dispatch only after the same prepared identity and canonical durable commit remain valid.

## 8. Durable Events, Snapshots, and Replay

- [ ] 8.1 Store immutable checksum-derived source/result snapshots and real artifact refs without mutating `ContextSnapshotStore.envelopes` in place.
- [ ] 8.2 Project planned/action/candidate/verified/rejected facts through the existing canonical Harness event/transcript port with bounded redacted metadata.
- [ ] 8.3 Make verified event append the activation commit boundary and fail closed when append fails.
- [ ] 8.4 Extend transcript/replay reports with pinned plan/action/result/admission/gate evidence and `side_effects_replayed=false`.
- [ ] 8.5 Verify record/snapshot/summary-artifact checksums and cross-refs during replay without invoking LLM, tools, memory writes, or publication.
- [ ] 8.6 Classify old `CompressionRecord`/snapshot projections as `legacy_unverified` and prevent them from authorizing recovery or dispatch.

## 9. Research Composition and Adversarial Tests

- [ ] 9.1 Inventory live Research context assembly/materialization callsites and bind the new runtime through the proper composition/application boundary.
- [ ] 9.2 Add group/materialization tests for role order, required output contracts, fake profile overrides, stable ids, and immutable source refs.
- [ ] 9.3 Add tool-transaction tests for orphan results, pending calls, complete atomic removal/replacement, and partial-action rejection.
- [ ] 9.4 Add planner/action tests for determinism, stale plans, reversible ordering, protected-only overflow, evidence selection, tool reduction, recent-tail preservation, and every execution bound.
- [ ] 9.5 Add summary tests for invented refs/outcomes, unsupported claims, missing required citations, conflicts, omission/loss policy, authority fields, and summary-call exhaustion.
- [ ] 9.6 Add aggregate VERIFY tests proving second physical gate failure cannot emit verified evidence or call a provider.
- [ ] 9.7 Add durability/replay corruption and crash-boundary tests for source/result/record/gate/artifact/event integrity and legacy classification.
- [ ] 9.8 Add Research integration tests covering evidence-first compaction, verified summary replacement, protected-context halt, and deterministic replay.

## 10. Validation and Delivery

- [ ] 10.1 Run focused Harness context/runtime/replay and Research context tests; fix root causes and retain adversarial evidence.
- [ ] 10.2 Run `python -m scripts.dev compile`, the broad non-live suite appropriate to shared Harness changes, and `python -m scripts.dev smoke`.
- [ ] 10.3 Run `openspec validate harness-context-compaction-verification --strict` plus relevant Change 1/cache validation where shared contracts are consumed.
- [ ] 10.4 Audit dependency direction, sensitive event/record projections, actual artifact refs, bounded state transitions, and the live dirty-worktree baseline.
- [ ] 10.5 Commit only this change's verified files with path-scoped staging, leaving concurrent cache/user modifications untouched.
