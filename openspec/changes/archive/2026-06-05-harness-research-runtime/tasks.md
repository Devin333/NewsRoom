...
- [x] 15.1 Delete old business modules that no longer serve Harness + Research.
- [x] 15.2 Delete old paper services, paper API routes, old worker queue bindings, and compatibility adapters replaced by Research.
- [x] 15.3 Delete or replace old paper/board tests only when old behavior is explicitly deprecated and Research coverage exists.
- [x] 15.4 Run `openspec validate harness-research-runtime --strict`, `python -m scripts.dev compile`, `python -m scripts.dev test`, and `python -m scripts.dev smoke`.
- [x] 15.5 Archive the OpenSpec change after all stages are complete.
