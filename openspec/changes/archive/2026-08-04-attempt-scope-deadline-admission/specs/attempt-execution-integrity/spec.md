...
**Reason**: A single shared `AttemptBudget` conflated each operation's local attempt number with the root retry ceiling and encouraged callers to reuse budget sequence values as resource ownership tokens.

**Migration**: Give every logical operation its own `LocalRetryBudget`, apply the root-scoped `RetryCreditLedger` only to retry starts, and keep resource leases owned by the protected resource.
