...
### 2.3 2026-08-20 framework live cutover audit

本节覆盖 2026-08-20 当前 working tree，并覆盖后文 append-only 历史 Gate 叙述。审计状态定义如下：

| 状态 | 含义 |
|---|---|
| `KEEP` | 本次必须保留的独立产品能力或 owner，不属于 legacy orchestration 删除范围 |
| `LIVE` | 当前 source 已有 production caller，并已进入 Graph-only live path |
| `WORKTREE_PENDING_GATE` | 替换已存在于当前 working tree，但尚未经过本轮完整 compile/smoke/strict/full-test 门禁和提交 |
| `OPEN` | 当前仍存在 active contract、identity、reader/writer 或 public surface 缺口 |
| `DELETE_PENDING_GATE` | source 已在 working tree 删除，但仍需 zero-import、public export、schema、test、smoke 和 strict validation 证明后才能算完成 |

当前事实纠正如下：

- flat `HarnessActivity` durable bridge、`HarnessControlPlane._process_graph_activity()` 和旧 `HARNESS_ACTIVITY_*` 执行路径已不在当前 source，不能再作为待实施现状；
- Research production composition 已构造并安装 `HarnessGraphPhysicalActivityExecutor`、`HarnessGraphPhysicalActivityDispatcher` 和 durable `HarnessNodeOutputResourcePort`；无 physical dispatcher 的 executable Graph admission、exact node-output result recovery 和 crash-window reconciliation 已进入 `WORKTREE_PENDING_GATE`；
- `GraphTerminalManifestV2` 与 `GraphExecutionVersionManifest` 已被 Artifact storage、Research publication/lifecycle、composition、inspection 和 Event projection 的 production caller 使用，状态为 `LIVE / KEEP`，不得删除或降级为 inactive contract；
- `framework/workflow/**`、`framework/harness/workflow/**` 和 `framework/specs/**` 的 Python source 已在 working tree 删除，production Python import scan 当前未发现这些 package 的 caller，但仍属于 `DELETE_PENDING_GATE`；
- Graph control plane 的 `_graph_result()` 已按 `node_instance_id` 聚合，且当前 source 未发现 control-plane 反向导入 `framework.harness.runtime`；canonical Harness side-effect intent/outcome 也已绑定 Graph physical identity，terminal 使用 `terminal_action`。剩余 P0 不再是重复重写这些 owner，而是消除 shared/runtime DTO 的弱身份、AgentLoop 直接 memory write、generic Artifact/conversation step authority、Tool/Worker/Budget 的可空 Graph scope，以及 standalone Paper RAG 伪造 Graph authority。

#### Framework 全量直接替换矩阵

| 优先级 | 状态 | owner / surface | 当前缺口 | 本变更必须完成的直接替换 | 完成证据 |
|---|---|---|---|---|---|
| P0 | `LIVE / REGRESSION` | `framework/harness/control_plane` result 与 `framework/harness/side_effects` | result 已按 `node_instance_id` 聚合；canonical side-effect checksum/match 已按 Graph/node/activity/attempt，terminal 使用 `terminal_action` | 保持这些 Graph-native owner，不再按旧审计重复重写；补同 definition 多实例、terminal、crash recovery 与 duplicate delivery 回归 | 两个相同 `step_id`、不同 `node_instance_id` 的结果和 side-effect slot 同时保留；terminal 不生成 synthetic node/step |
| P0 | `OPEN` | `framework/shared/graph_identity.py` 与所有 runtime carrier | `GraphExecutionIdentity` 已表达 physical activity，但 `GraphRunIdentity` 只有 run/Graph 和可空 node-instance/stage；Event、Worker、Memory、Skill 等把弱类型当执行事实 | 建立 discriminated run/stage/execution/standalone scope；Graph activity facts 强制 `GraphExecutionIdentity`，terminal 使用 run-level identity，独立能力显式 standalone | nullable/mixed scope、跨 Graph/checksum/node/activity/attempt substitution 全部 fail closed |
| P0 | `OPEN` | `framework/agent/loop` 与 `framework/memory/integrations` | AgentLoop 仍在 tool/final output 后直接调用 memory runtime；Graph memory adapter 也暴露 direct write 且只使用弱 run identity | AgentLoop 只返回 bounded memory candidate/evidence；只有 Harness deterministic gate 与 controller-terminal side-effect handler 可以 commit/consolidate memory | worker 阶段 memory-write count 为零；VERIFY 失败、replay、duplicate delivery 和跨 identity tamper 均不写 memory |
| P0 | `OPEN` | Paper RAG / Context application boundary | standalone `PaperRAGService` 自行生成 `run_id` 和 stage identity 后直接运行 RAG，未进入真实 Harness Graph activity；stage identity 与 physical identity 仍混在 nullable DTO | 分离 immutable `GraphStageIdentity` 与 `GraphExecutionIdentity`；Graph-bound Research RAG 注入真实 activity identity，独立 RAG 使用显式 standalone scope | standalone 不伪造 Graph authority；Graph RAG 的 context/cache/evidence 在 retry/parallel 下唯一归属 |
| P0 | `WORKTREE_PENDING_GATE` | admission / dispatch / node output / recovery | Research 已安装唯一 physical dispatcher；当前新增了 pre-mutation admission、exact current-commit recovery 和 dispatched-without-result reconciliation，但尚未完成本轮总门禁 | 保持 control plane 只做 durable decision、dispatch、VERIFY 和 transition；physical executor 独占 worker execution、lease、cancellation、reconciliation 与 result commit；无 dispatcher 或无 exact commit 时 fail closed | 无 dispatcher 时零 durable mutation；restart/crash/cancel/lease-fencing/duplicate-delivery 测试通过；同一 activity 不会双重执行 |
| P0 | `OPEN` | Event canonical identity / budget | `BusinessContext` 没有 `node_instance_id`；budget sink 只写 `run_id`；`framework.shared.graph_identity.GraphRunIdentity` 与 `framework.events.projection.GraphRunIdentity` 同名且字段不兼容 | 建立唯一 canonical `GraphRunIdentity`；execution-version/projection 字段使用明确命名的 manifest/DTO；Event、budget、projection 和 durable Graph extension 显式转换并校验 | 同一 normalized Graph 的 Event、Trace、Memory、Skill、Artifact manifest identity 全字段一致；同 run 的多个 node budget 可独立查询和 replay |
| P1 | `OPEN` | Trace / Tool / LLM observability | `TraceContext`、`trace_fields()`、`ToolEvent` 缺少 `node_instance_id`；`StructuredOutputEvent` 只有 nullable `run_id` | 传播 exact Graph/node/activity/attempt identity；序列化、carrier、child trace、tool event、structured-output validation/repair metrics 全部闭合 | 并行或重复 step 的 trace/tool/LLM facts 往返后仍能唯一归属 activity attempt |
| P1 | `OPEN` | AgentLoop / SubAgent / TaskPlan / Agent message | Graph-bound AgentLoop、SubAgent v3 和 durable TaskPlan contract 已存在；但 message persistence 仍以 `step_id` 为主，外围 consumer 仍需按实例级 contract 收口 | 保留 Graph 内的 bounded dynamic TaskPlan 和 AgentLoop；删除 live schema fallback；conversation/message、cursor、checkpoint、transcript、receipt 和 result 全部绑定 exact node instance | Graph-bound AgentLoop/SubAgent/TaskPlan production E2E 经过 physical activity -> node output -> deterministic VERIFY；retry/loop message 不混淆 |
| P1 | `LIVE / KEEP` | `framework/harness/artifacts` 与 physical Artifact storage | `GraphTerminalManifestV2` live writer/reader 已激活；剩余工作是更新仍断言“v2 inactive”的测试，并确认 v1 仅可 quarantine | 保留 Artifact owner、integrity、catalog、governance、GC、inspection、storage 和 controller-terminal publication；删除 v1 live reader/writer 与 Workflow bridge | V2 strict read/write/read-back、execution-version/checksum、publication gate、catalog/GC/inspection 和 production composition tests 全通过 |
| P1 | `OPEN` | Wait / approval / API / CLI / MCP / SDK | `/api/v1/approvals/{id}/resume-context`、`buffer_updates`、`resume_metadata`、CLI `DataBuffer` 文案和旧 approval client surface 仍是 active external contract | 直接发布 typed Graph Wait-cause/approval application contract；删除共享 buffer/state patch；cause durable commit 后只由 Harness 自动 resume | 外部面只接收 Graph run/node/wait identity 与 bounded cause；旧 endpoint/字段/client method 零引用 |
| P1 | `OPEN` | Memory / Governance / Budget / Worker / Skill | worker/skill/memory 已有 Graph identity contract，但 `GlobalBudgetTracker` 仍可用 `legacy-global-budget` 等默认 scope 创建；部分 live alias 仍绕过统一命名 | production entry 要求 explicit Graph-bound scope；删除 active legacy defaults/schema fallback；worker 继续只能提交 candidate/evidence，不能决定 routing、memory write、promotion 或 publication | 无 Graph scope 的 production tracker/admission fail closed；PostgreSQL、worker、memory、skill、governance facts 使用同一 identity |
| P1 | `OPEN` | RAG / Context | Graph-bound `ContextEnvelope` 与 Research caller 已存在；无 caller 的 `context_payload()` facade 和两个 `legacy_unverified` replay reader 仍被导出 | 保留 RAG session、context compaction 和 V2 durable replay；删除无 caller compatibility facade，把 legacy replay reader 移出 production import 后删除 active export | Context snapshot/cache/materializer/RAG caller 均绑定 exact Graph identity；compat facade/public export 零引用，V2 replay 继续通过 |
| P2 | `LIVE / REGRESSION` | Tool registry memory naming | production registry、MCP 与 smoke 当前只使用 `memory.recall`，`memory.search` alias 已删除 | 保持唯一 canonical name，并增加禁止旧 alias 恢复的 architecture/contract regression | `memory.search` 被 deterministic reject；`memory.recall` authorization/read path 正常 |
| P2 | `DELETE_PENDING_GATE` | legacy packages / root exports / history readers | Workflow/spec source 已删除，但 root `__all__`、architecture freeze baseline、fixtures、ignored bytecode、history/migration reader 仍需逐项证明不会恢复旧 authority | 删除旧 package public surface 和仅验证旧 runtime 的 tests/fixtures；历史数据只允许隔离识别和 typed quarantine，不能进入 live resume/replay/worker/side-effect | production import/public schema/entrypoint zero-reference；history input 零 worker、零 side effect；source validation、smoke、strict validation 全通过 |
| P2 | `OPEN` | canonical OpenSpec / architecture tests | 三个 architecture tests 仍要求 Graph Artifact/NodeOutput“未激活”，一个 test 搜索已删除的 `_process_graph_activity()`；canonical requirements 和 approval文档仍有旧 surface | 把 inactive assertions 改成 positive production activation/owner tests；删除或 supersede Workflow requirements；同步 Graph approval/storage/indexing contracts | `openspec validate graph-only-orchestration --strict`、`openspec validate --all --strict`、architecture suite 和 stale-reference inventory 全通过 |

#### Execution identity 不变量

`step_id` 不是一律删除的 legacy token。`HarnessStepSpec.step_id`、`StepRef` 和 Graph compiler/scheduler 中的 `step_id` 可以继续作为 **Graph definition-level leaf identifier**。但任何 durable execution、result、event、trace、budget、approval、message、tool fact、side effect 或 Artifact lineage 都不得只用 `step_id` 作为权威身份；至少必须闭合到 `run_id + graph_ref + graph_checksum + node_instance_id`，activity 级事实还必须携带 `activity_id + attempt`。因此 zero-reference gate 针对的是 flat orchestration descriptor、Workflow authority 和把 `step_id` 当唯一 execution identity 的 caller，不要求机械重命名 Graph DSL 内每个 `step_id`。

这里的“framework 全量”按 authority、identity 和调用链定义，不要求把 scheduler/evaluator 文件物理移动到 `framework/harness/graph`。`framework/harness/control_plane` 可以继续作为 Graph control-plane 的实现 owner；不可保留的是 Workflow identity、flat activity bridge、第二个 worker execution authority、v1 Graph manifest live reader、active compatibility facade 或缺失 node-instance identity 的持久化/public contract。普通业务 fallback、context compaction 和 source reference 缺失诊断不属于旧 orchestration fallback；history quarantine 只能位于非 production execution 路径，且不得提供 rollback 或恢复旧 runtime 的能力。

当前验证状态必须以本轮结果为准，不复用后文历史快照。最近一次本变更 targeted evidence 为 `7 failed, 91 passed, 2 skipped`：失败集中在已删除 run-operation/step API 的旧 MCP assertions、AgentLoop smoke 的 committed-output ref mismatch，以及仍断言 step-based Artifact 行为的测试。Research Artifact v2 reader focused suite 为 `49 passed, 2 skipped`，但 recovery composition 仍有 `3 failed`，包括旧恢复记录缺失新 Graph identity 与错误契约未同步。`openspec validate graph-only-orchestration --strict` 和 strict-all 历史上已通过，但当前 working tree 仍必须重新运行 `compile`、mandatory `smoke`、focused/full tests 和 strict validation；在这些失败根因修复并全部重跑前不得声称 Graph-only cutover 已完成。
...
Task 3.3 的 live audit 曾区分“已有 durable phase event”和“已经绑定完整 Graph identity”。Gate A strict contract 由 commit `a368baee4d13108a27705ebc3134a5e563550a0d` 落地：`framework.events.graph_phase.GraphPhaseTransitionRecord` 复用 event-owned `GraphRunIdentity` / `GraphEventContext`，强制 node scope、exact versions、封闭 phase/boundary、非负 attempt、正 event sequence、唯一 canonical-order SHA-256 gate evidence refs、timezone-aware UTC time、record checksum 与 envelope-sequence assertion；strict reader 对未知字段、Workflow alias、moving version、非 canonical refs 和完整性篡改 fail closed。

production activation 已由 commit `cf07ca788d7c5b235e47cef1fb875e93b5bf9196` 完成。`HarnessControlPlane._record_graph_phase()` 只经过 `record_graph_phase_transition()`，该路径由 `DurableGraphEventContextWriter` 校验 canonical stream、exact Graph context/business identity、CAS sequence，并在 commit 后读回验证 context 和 envelope sequence；generic durable record 不能绕过 phase writer。`DurableHarnessTransitionPort` 的 recovery/replay commit 复用同一 writer，`EventProjectionService` 通过 `GraphEventProjectionApplicationPort` 验证 Graph projection，inspection 使用 Graph application services、canonical index 与 durable history，不再接受 legacy reader/fallback。Task 3.3 与 4.11 已完成；机器可读证据位于 `evidence/graph-phase-transition-contract.json` 与 `evidence/event-application-port-contract.json`。本 slice 没有删除或改变 Artifact owner/runtime/storage/publication authority。
...
`context` 必须同时包含 `node_id` 和 `node_instance_id`；phase record 不接受 run-only context。`event_sequence` 是对应 durable Graph run stream 的正整数序号，active `DurableGraphEventContextWriter` 强制它与 stored event envelope 的 `stream_sequence` 相同。`gate_evidence_refs` 只接受唯一、canonical-order 的 SHA-256 refs；record checksum 覆盖完整 schema、context、phase/boundary、attempt、sequence、evidence refs 和 UTC time。legacy `phase_recorded` writer/reader 不得绕过此路径或成为 external projection 的输入。
...
> **2026-08-24 live 状态校正**：上一段及其前一段属于历史快照，关于 `legacy phase_recorded`、Task 3.3、Task 4.11、Task 4.12 和 Task 4.13 的旧 blocker 表述已被后续实现取代。commit `cf07ca788d7c5b235e47cef1fb875e93b5bf9196` 激活了 durable Graph context writer、Graph index reader/inspection/replay authority，并删除 inspection fallback；Task 3.3、4.11、4.12 与 4.13 均以 `tasks.md` 当前 checklist 为准。该进展不影响仍开放的 compiler/admission、Research、public interfaces、history cleanup 与 release tasks，也不构成最终 Graph-only release 声明。
...
## 50. Framework 全量 Graph-only 影响面审计（2026-08-19）

本节是对当前 working tree 的 live source audit，不是历史 Gate A 记录。审计结论是：本次不是简单把 `HarnessRunSpec.workflow` 改名为 `graph`，而是要把所有“执行事实的归属键”从 definition-level `step_id` 收敛为 immutable Graph identity + `node_instance_id` + activity/attempt lineage。`step_id` 仍可作为 Graph DSL 的静态 node/step 定义标识和 binding lookup key，但不得再作为 durable result、approval、message、artifact lineage、recovery slot 或 public inspection 的 authority。

### 50.1 已经对齐的 live surface

以下 surface 在当前 source 中已经使用 Graph contract，后续只需保持回归，不应重新引入 Workflow facade 或 step-based fallback：

| Surface | 当前 owner / 证据 | 结论 |
|---|---|---|
| Run declaration | `framework/harness/control_plane/state.py` 的 `HarnessRunSpec.graph` | Graph 是唯一外层声明入口；legacy/dual payload 在 admission 前拒绝 |
| Graph compile/preflight | `framework/harness/graph/compiler.py`、`framework/harness/graph/validation/**` | 只接受 pinned Graph definition、exact contract 和 checksum |
| Control-plane result | `framework/harness/control_plane/harness.py:_graph_result` | `worker_results`、gate/quality cache 按 `node_instance_id` 保存；不得恢复 `step_id -> latest` |
| Wait / approval resume | `framework/harness/waits/**`、`graph_application.py` | wait scope 已含 `node_instance_id`；cause 由 Harness durable commit 后路由 |
| Event / trace envelope | `framework/events/**`、`framework/shared/graph_identity.py` | canonical identity、execution version、node instance 已进入 envelope；禁止新增第二个 identity owner |
| Terminal Artifact | `framework/harness/artifacts/**`、`infrastructure/research/artifact_{port,publication}.py` | `GraphTerminalManifestV2`、execution-version、event projection 走 Artifact owner；Artifact subsystem 必须保留 |
| Public run event API | `interfaces/services/event_reader_service.py`、`run_inspection_service.py`、API/CLI/MCP/SDK | 过滤键已切为 `node_instance_id`；不得重新暴露 `step_id` authority |
| AgentLoop cursor/checkpoint | `infrastructure/storage/conversation/models.py` 的 cursor/checkpoint v2 | outer identity 已是 Graph run/node/checkpoint；message record 仍未完成同级迁移（见 50.2） |

### 50.2 必须直接替换的框架缺口

| 优先级 | owner / 文件 | 当前问题 | 直接替换要求 | 必测场景 |
|---|---|---|---|---|
| P0 | `framework/harness/side_effects/approval.py`、`side_effects/models.py`、`control_plane/harness.py` | approval request/evidence、intent、decision 仍把 `run_id + step_id + attempt` 当事实身份；terminal 使用 `__terminal__` synthetic step；重复 step 会覆盖 side-effect slot | 明确保存 `graph_id/version/ref/checksum`、`node_id`、`node_instance_id`、activity/attempt；`step_id` 只能作 definition label。worker slot、recovery、replay、checksum、resolver 全部按 node instance；terminal 使用 run-level Graph + terminal action，不造 step token | 同一 `step_id` 的 loop/retry/parallel 两实例同时授权、跨 Graph tamper、terminal approval、crash recovery、重复 delivery 不覆盖 |
| P0 | `framework/harness/control_plane/harness.py` 与 Graph runtime boundary | control plane 仍直接持有部分 physical execution contract，形成 control-plane -> runtime 反向依赖风险 | execution input、task context、node-output/result port 的 contract 归 Graph/shared owner；control plane 只做 durable decision/dispatch/VERIFY，physical executor 独占 worker execution、lease、cancellation、reconciliation | architecture import gate、无 dispatcher 零 mutation、crash window、lease fencing、duplicate delivery |
| P0 | `infrastructure/storage/conversation/models.py`、`local_json.py`、`postgres/conversation.py`、`postgres/migrations/**` | `AgentMessageRecord`、parent conversation upsert、SQL/JSON redaction 仍写 nullable `step_id`；cursor/checkpoint 已 v2，造成同一 conversation 内两套身份 | Graph-bound message 改为 `run_id + graph_ref/checksum + node_instance_id + graph_checkpoint_ref + activity/attempt`；standalone conversation 明确无 outer Graph scope；删除 active `step_id` 列、查询和 index，执行 major schema migration | 同 step 不同 node instance 消息隔离、retry/loop 顺序、local/PG round-trip、跨 Graph tamper、reopen/replay |
| P0 | `framework/agent/artifacts/models/reference.py`、`framework/agent/artifacts/stores/filesystem.py`、`infrastructure/storage/artifacts/local_json.py`、`postgres/artifact_index.py`、`postgres/migrations/**` | generic `ArtifactRef`、路径分区、`list_by_step()` 和 Postgres index 仍以 `step_id` 做 lineage/key；Research V2 manifest 的 Graph metadata 无法修复 generic index 丢身份 | Artifact lineage 改为显式 Graph identity + `node_instance_id`，activity artifact 再加 `activity_id + attempt`；路径、去重、查询、GC 使用 node instance；`list_by_step()` 删除或降为非权威诊断过滤，替换为 exact node-instance 查询 | 同 step 两实例写/读/delete/GC 不串线、跨 run/Graph checksum fencing、manifest read-back、duplicate artifact idempotency |
| P1 | `interfaces/services/run_inspection_service.py` | `/steps` 仍从 manifest 的 `terminal_node_ids` 生成 definition-level 列表，无法表达 loop/retry/branch 的实际实例 | inspection 读取 durable Graph event/state projection，返回 `node_id + node_instance_id + attempt + status + terminal`；无 projection 时返回 typed unavailable，不回退 manifest 猜测 | 并行同 definition、多次 retry、事件分页、projection unavailable、replay 不调用 worker |
| P1 | `framework/events/schema/security.py`、Tool/LLM/Memory/Skill/Worker observability | schema security allowlist 和若干外围 record 仍允许/携带 `step_id` 或 Workflow scope；部分 metrics 只带 run | active event/trace/tool/LLM/memory/skill/structured-output record 全部传递 exact Graph/node/activity/attempt；`step_id` 仅保留 DSL/诊断 allowlist；明确 standalone scope | trace/tool/LLM facts 在 retry/parallel 后唯一归属；unknown alias、scope mismatch、redaction |
| P1 | `framework/harness/task_plan/**`、SubAgent transcript/result/replay | Graph v2 contract 已分批建立，但 queue/checkpoint/recovery/projection 的 active caller 仍需统一验证，不能让 v2 result 落回旧 identity | PLAN/EXECUTE/VERIFY 全链路只接受同一 frozen Graph stage identity；durable event、result、transcript、queue、checkpoint、replay 使用同一 checksum；缺 schema 直接 fail closed | replan/retry budget、crash reopen、跨 Graph/task substitution、zero worker on replay |
| P1 | Research composition / RAG / context | Research production binding、context pack、reader repair、dynamic TaskPlan 仍需确认每个 caller 都从 Graph identity 注入，不能依赖 nullable session/legacy handler | composition 只安装 exact Graph physical dispatcher、activity/output/artifact/event ports；RAG/context 用 run/node/checkpoint identity；repair memory 只经 Harness terminal side effect | static/dynamic/reader repair、actor isolation、gated failure、artifact publication、recovery |
| P2 | API/CLI/MCP/SDK schemas、public exports | 部分旧 names、compatibility handler、history reader 和 generated schema 仍可能从 active import path 到达 | 删除 forwarding facade、fallback executor、legacy writer/reader、synthetic terminal field；新 major schema 只输出 Graph identity；历史输入只能 typed quarantine | import graph zero-reference、OpenAPI/CLI/MCP/SDK contract、unknown/legacy payload rejection |

### 50.3 明确保留的内容

- `HarnessGraphNode.step_id`、Graph definition 的静态 binding key、编译器错误定位和离线诊断字段可以保留；它们不是运行实例 authority。
- `framework/harness/artifacts/**`、Artifact body store、terminal manifest、catalog、integrity、governance、GC、inspection、publication 和 Research Artifact owner 必须保留。删除目标只包括 legacy Workflow Artifact bridge/writer/reader 与 `ARTIFACT` leaf classification。
- 独立 conversation/RAG session 可以没有 outer Graph identity，但必须显式声明 standalone scope，不能把 nullable `step_id` 当作隐式 fallback。
- history-only Workflow record/classifier 可以保留在隔离工具路径；不能被 production composition import，也不能 resume/replay/dispatch/写 memory/发布 Artifact。

### 50.4 实施顺序与停止条件

实施顺序固定为：

1. 先冻结 canonical `GraphRunIdentity` 和 node-instance/activity/attempt 字段，完成 side-effect approval/intent/decision/outcome 的 schema 与 slot 改造。
2. 再迁移 conversation message 和 generic Artifact index/storage；完成 local/PostgreSQL major schema、路径、查询和 read-back。
3. 接着切换 event/trace/tool/LLM/memory/TaskPlan/inspection 的 read/write projection，并补齐 Research production composition、physical executor、node-output 和 recovery。
