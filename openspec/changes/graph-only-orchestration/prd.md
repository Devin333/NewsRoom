...
本节覆盖 2026-08-19 当前 working tree，并覆盖后文 append-only 历史 Gate 叙述。审计状态定义如下：
...
- Graph control plane 仍有真实边界缺口：公开 run result 按 `step_id` 压扁实例结果、control plane 反向导入 runtime execution contract、Graph identity 在 Event 与 shared owner 中重复定义，外围 observability 和 approval contract 也尚未完成实例级切换。

#### Framework 全量直接替换矩阵

| 优先级 | 状态 | owner / surface | 当前缺口 | 本变更必须完成的直接替换 | 完成证据 |
|---|---|---|---|---|---|
| P0 | `OPEN` | `framework/harness/control_plane` public result | `HarnessRunResult.worker_results` 和 `quality_verdicts` 在 `_graph_result()` 中按 `node.step_id` 写入；loop、retry、compensation 或重复逻辑 step 会互相覆盖 | 权威结果按 `node_instance_id` 保存；definition-level step 视图只能由显式只读 aggregation 生成，不得覆盖实例结果 | 两个相同 `step_id`、不同 `node_instance_id` 的结果同时保留；inspection、Research diagnostics、API projection 和 replay 全部实例级一致 |
| P0 | `OPEN` | control-plane / physical-runtime contract owner | `HarnessControlPlane` 从 `framework.harness.runtime.activity_executor` 导入 `HarnessGraphActivityExecutionInput` 等执行契约，形成 control-plane -> runtime 反向依赖 | 将 activity execution input/task context/port 移到 control-plane 或 shared contract owner；runtime executor 只实现这些 port，不反向拥有控制面 contract | architecture import gate 通过；control plane 不导入 `framework.harness.runtime`，production dispatcher/executor 行为不变 |
...
| P2 | `OPEN` | Tool registry compatibility | `memory.search` deprecated alias 仍注册在 production tool registry，并被 MCP 与 AgentLoop smoke 使用 | 直接统一为 `memory.recall`；删除 registry alias、MCP alias、旧 smoke fixture 和文档 | `memory.search` 被 deterministic reject；`memory.recall` 的 authorization、read path 和 tests 正常 |
...
当前验证状态必须以本轮结果为准，不复用后文历史快照。2026-08-19 已复核四个相关 architecture modules，结果为 `16 passed, 4 failed`：`test_graph_artifact_owner_boundary.py` 和 `test_graph_node_output_owner_boundary.py` 仍把 live V2/physical runtime 误判为 inactive，`test_harness_durable_event_boundary.py` 仍搜索已删除的 `_process_graph_activity()`，这三项需要更新；`test_harness_graph_result_boundary.py` 暴露 control-plane -> runtime import 的真实边界缺口，必须修代码。`openspec validate graph-only-orchestration --strict` 和 `openspec validate --all --strict` 已通过，后者为 `533 passed, 0 failed`；当前 working tree 尚未完成本轮 `compile`、mandatory `smoke` 和 full test，因此在这些门禁全部重跑前不得声称 Graph-only cutover 已完成。
