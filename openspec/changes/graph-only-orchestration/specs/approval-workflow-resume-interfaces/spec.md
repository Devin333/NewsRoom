...
**Migration**：发布 Graph resume major endpoint，并在调用方清单完成后删除旧 endpoint。

### Requirement: CLI resumes workflow from approval

**Reason**：`resume-workflow` command 暴露已退役的执行模型。

**Migration**：发布 Graph resume command，并在调用方清单完成后删除旧 command。

### Requirement: MCP and SDK resume workflow from approval

**Reason**：MCP/SDK 旧 contract 依赖 Workflow identity。

**Migration**：发布 Graph resume MCP tool 和 SDK method，并在 major cutover 后删除旧 surface。
