...
- **THEN** reading the checkpoint returns the same conversation id, agent id, run id, node-instance id, Graph checkpoint id, status, iteration count, stop reason, trace summary, diagnostics summary, LLM artifact ids, and metadata
