...
- Baseline inspected: branch `main`, `HEAD 41e4833c`.
...
- Task ledger at audit start: `0/41`. After the implementation and evidence
  ...
  `41/41` complete. Tasks `3.7`, `3.10`, and `7.5` are complete. A task is marked complete only
...
- The Source core implementation, tests, persistence corpus, and this evidence
  file are isolated as one Source-owned candidate. The current composition
  candidate is replayable; the parent Harness change still has independent
  execution-environment and external-release gates.
...
- The core URL, retry, taxonomy, error-factory, mapper, default Research binding,
  entry-surface quota retention, and typed Research package/PDF denial have
  passing focused evidence. Harness Source capability is explicitly unsupported
  in this release because no canonical Harness-owned Source `ToolPort` is
  composed; Harness must not admit that capability until such a binding exists.
...
Current observed result: `open=0 done=41 total=41`. The pre-corpus baseline was
...
| Default Source runtime composition shares state | `test_source_runtime_composition.py` and the default-policy Research arXiv denial test | Passing: API, MCP, worker health, CLI, ToolApplicationService, and Research package/PDF paths reuse the intended provider; Harness Source capability is explicitly unsupported |
...
| Source error artifacts preserve refs/request ids | artifact/index writer tests, concurrent request-context tests, forced connector response-metadata interleaving, and bounded one-shot protocol pending-state tests | Passing working-tree contract; final archive/release qualification remains a parent-change gate |

## 3. Composition gates and known incomplete work

The Source core and production entry owners are intentionally evaluated as
separate gates. The current candidate now has default-provider tests in
addition to explicit injected-object tests.

| Gate | Current code evidence | Missing proof / decision | Disposition |
| --- | --- | --- | --- |
| Core Source composition | `interfaces/services/source_runtime.py` creates one `DomainRateLimiter` and injects it into connectors, Source tool runtime, health/service adapters, and `research_arxiv_connector`; `SourceRuntimeProvider.get()` is lock-protected; connector response metadata uses request-local `ContextVar` state; `BasicSourceHealthManager` serializes in-process state transitions | `tests/interfaces/services/test_source_runtime_composition.py` covers concurrent provider construction, cross-entry quota denial, and ToolApplicationService binding | Passing working-tree integration gate |
| API | `interfaces/api/app.py` resolves the process-owned default Research Source provider when the default Source factory is selected, and passes it to worker and MCP factories; the provider is process-scoped rather than closed with each FastAPI app | `test_api_default_source_runtime_retains_quota_and_denies_before_network`; recorded transport parity in `tests/interfaces/composition/test_research_recorded_transport.py` | Passing entry-surface gate |
| MCP | `interfaces/services/mcp_service.py` resolves the process-owned default Research Source provider for the default service and binds worker and Research factories to it; explicit providers remain injectable and isolated | `test_mcp_source_runtime_retains_quota_and_denies_before_network`; `test_mcp_default_worker_factory_receives_the_mcp_source_provider`; recorded transport parity | Passing entry-surface gate |
| Worker | `interfaces/services/worker_service.py` accepts one provider and uses its service for default Source health handlers; all worker CLI commands forward parser provider | `test_worker_source_health_uses_provider_ledger_before_probe`; `test_cli_worker_reuses_parser_source_runtime_provider` | Passing entry-surface gate |
| CLI | `interfaces/composition/source.py` owns provider creation; `interfaces/cli/news.py` installs one parser-lifetime provider for sources, tools, workers, MCP, and paper commands | `test_cli_source_runtime_retains_quota_and_denies_before_network`; `test_cli_parser_installs_one_lazy_source_runtime_provider` | Passing entry-surface gate |
| Source tool application | `ToolApplicationService` builds the business registry from every component of the resolved composition | `test_tool_application_service_builds_source_registry_from_provider`; `test_connector_tool_and_health_probe_share_one_reservation_ledger` | Passing entry-surface gate |
| Research arXiv package/PDF | `SourceRuntimeComposition.research_arxiv_connector` uses the shared ledger; default `paper_rag_factory` resolves the provider from the default Research composition; configured Research composition injects the same package connector into document compilers | `tests/interfaces/services/test_source_research_rate_limit.py`; `test_chunk_pipeline_default_source_runtime_comes_from_research_provider`; production composition assertions in `tests/interfaces/composition/test_research_composition.py` | Passing Research composition gate |
| Harness Source capability (`U2`) | Repository search finds no Harness-owned `SourceRuntimeProvider`, Source `ToolPort`, or canonical production Source registry binding | Explicit release decision: Harness Source capability is unsupported and must remain outside active TaskPlan/tool bindings until a canonical Harness-owned adapter is composed. No second registry or fake path is admitted. | Explicitly unsupported; not a completion claim |
...
standalone adapter test fallback. The second locates `ToolApplicationService`,
but no production Harness Source composition owner; that absence is the
recorded unsupported-capability decision above.
...
### 8.0 Current composition candidate verification

Executed from `F:\github\NewsRoom` with the repository virtual environment on
`2026-08-25`:

```powershell
.\.venv\Scripts\python.exe -m pytest tests/architecture -q
# 221 passed, 8 warnings
...
.\.venv\Scripts\python.exe -m pytest `
  ...
  tests/interfaces/cli/test_cli_worker_commands.py `
  tests/interfaces/cli/test_worker_commands.py `
  tests/interfaces/services/test_worker_app_service.py `
  tests/interfaces/composition/test_research_composition.py -q
# 118 passed, 13 warnings

git diff --check
# passed; Git reports only the existing CRLF-to-LF normalization warning for
# interfaces/cli/news.py
```

The mandatory compile, smoke, strict OpenSpec, and diff checks were rerun on
the complete candidate. The path-scoped commit is recorded after staging below.
...
# Research/Harness/architecture: 2318 passed, 23 deselected, 22 warnings;
# offline AgentLoop smoke succeeded; Source validation is_valid=true
...
### 8.1 Historical Source-core verification record
...
The historical Source-core record above is retained for provenance. Current
composition verification is in section 8.0. Release qualification still
requires the parent Harness execution-environment gates and the rollback
fields in section 6; those are separate from the Source contract evidence.
...
| Default Research shares the process Source ledger | Process-local quota sufficiency across hosts is not proven by unit tests | Keep the process-scope contract; qualify distributed deployment topology separately and retain the default Research composition/no-network denial tests. |
