...
## Purpose
Define bounded, scope-aware retry and deadline admission across Workflow, Tool,
...
#### Scenario: Tool retry does not increment its parent Step
- **WHEN** a Step local attempt 1 invokes a Tool whose own local attempt advances from 1 to 2
- **THEN** the Tool local attempt number becomes 2, the Step local attempt remains 1, the Tool reuses its logical idempotency key, and exactly one root retry credit is consumed
...
- **WHEN** a Step requests a `DataBuffer` write lease after successful admission
- **THEN** the lease contains the resource-issued generation and owner attempt ID, and a budget sequence or retry credit cannot be used as that generation
...
Workflow Steps, parallel branches, Tool calls and batches, nested Workers, and standalone timeout helpers SHALL create explicit logical operation contexts and route physical starts through the shared admission contract. Sibling operations SHALL not share local attempt numbers or idempotency keys; queue/storage lease ownership SHALL remain outside execution retry identity.

#### Scenario: Cross-layer scope matrix is isolated
- **WHEN** a test exercises a direct Tool, Workflow Tool, ToolBatch child, parallel branch, nested Worker Tool, and standalone timeout helper
...
#### Scenario: DataBuffer lease is acquired after Step admission
- **WHEN** a Step request is rejected for deadline or capacity before start
- **THEN** `DataBuffer.begin_attempt` is not called and the current valid owner remains unchanged

### Requirement: Retry safety and determinacy take precedence over remaining budget
The runtime SHALL preserve read-only, idempotent-write, reconciliation, and no-effect error contracts. `INDETERMINATE`, unconfirmed termination, unsafe external-write failure, and an indeterminate descendant SHALL block retry and normal commit/publication even when local and root budgets remain. A composite runner with a conservative external-write capability MAY preserve `FAILED` only when its trusted child outcomes explicitly confirm both termination and effect determinacy; absence of that proof SHALL remain fail-closed.
...
- **THEN** the parent becomes indeterminate and commits no normal buffer or artifact output regardless of remaining retry credits

#### Scenario: Read-only composite child failure remains determinate
- **WHEN** a ToolBatch runner has a conservative external-write capability but every failed child is read-only, terminated, and explicitly reports a determinate effect
- **THEN** the Step terminal remains `FAILED` rather than being upgraded to `INDETERMINATE`
