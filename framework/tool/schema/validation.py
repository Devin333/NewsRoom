...
class ToolArgumentValidator:
    def validate(self, definition: ToolDefinition, arguments: dict[str, Any]) -> list[str]:
        try:
            normalize_tool_arguments(definition, arguments)
        except (ToolDefinitionError, ToolRuntimeError) as exc:
            return [str(exc)]
        return []

    def validate_or_raise(self, definition: ToolDefinition, arguments: dict[str, Any]) -> None:
        normalize_tool_arguments(definition, arguments)
...
def normalize_tool_arguments(
    ...
    missing = [argument for argument in definition.required_arguments if argument not in normalized]
    if missing:
