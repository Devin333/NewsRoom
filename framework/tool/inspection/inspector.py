...
def classify_tool_risk(definition: ToolDefinition) -> str:
    if _is_dangerous_tool(definition) or definition.side_effect_value == "destructive":
        return "critical"
    if definition.side_effect_value in {
        "publishing",
...
def _has_side_effects(definition: ToolDefinition) -> bool:
    return definition.side_effect_value not in SIDE_EFFECT_NONE
