...
def build_main_overlap_span_metadata(
    *,
    content: str,
    overlap_text: str = "",
...
def remap_span_origin_ids(metadata: Mapping[str, Any], id_map: Mapping[str, str]) -> dict[str, Any]:
    out = dict(metadata)
...
def resolve_source_span(
    *,
    ...
    if (resolved_start is None or resolved_end is None) and snippet:
        resolved = locate_snippet_span(content, snippet)
    ...
    if selected_span.get("kind") == "overlap":
        return SourceSpanResolution(
            ...
            resolved_chunk_id=str(selected_span.get("origin_chunk_id") or chunk_id),
            resolved_source_locator=str(selected_span.get("origin_source_locator") or source_locator),
            span=selected_span.get("span") or {},
        )
    return SourceSpanResolution(
        span_kind="main",
        chunk_id=chunk_id,
...
def locate_snippet_span(content: str, snippet: str) -> tuple[int, int] | None:
...
def _remap_span_origin_id(span: Mapping[str, Any], id_map: Mapping[str, str]) -> dict[str, Any]:
    out = dict(span)
