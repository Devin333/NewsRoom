...
| 20 | [20-framework-boundary-and-duplication-convergence.md](20-framework-boundary-and-duplication-convergence.md) | `READY_FOR_OPENSPEC` | `IN_PROGRESS` | 2026-07-18 框架层与重复实现专项审查；Harness 切片已实现并归档；Source 为 38/41；Research implementation `12ed843a` 已按 staged-only verified tree 完成 46/46，change 尚未归档；复用 `framework-runtime-safety-hardening`、`durable-event-runtime` | Source 的 3.7、3.10、7.5，Research 的 RES-007、U7，以及阶段级 U9 保持开放；随后收敛 Harness side-effect authority、Tool approval/policy、Quality/Workflow contracts。budget/conversation/memory 与 legacy 候选必须先通过证据门，不能凭目录或字段相似性直接合并/删除。 |
