...
| 20 | [20-framework-boundary-and-duplication-convergence.md](20-framework-boundary-and-duplication-convergence.md) | `READY_FOR_OPENSPEC` | `IN_PROGRESS` | 2026-07-18 框架层与重复实现专项审查；Harness gate 切片已实现并归档；Source 为 38/41；Research implementation `12ed843a` 已按 staged-only verified tree 完成 46/46，change 尚未归档；HAR-008..009 的 `harness-side-effect-authority-closure` 已 apply-ready（51 项 tasks 未开始）；复用 `framework-runtime-safety-hardening`、`durable-event-runtime` | 先按 `decision -> effect -> durable outcome -> success` 收敛 Research hidden artifact preparation、controller-terminal atomic publication 和 run disposition/latest；generic Tool/Memory 与 production skill release 仍由各自 owner 显式接入。Source 的 3.7、3.10、7.5，Research 的 RES-007、U7，以及阶段级 U9 保持开放；Quality/Workflow、budget/conversation/memory 与 legacy 候选继续按证据门拆分，不能凭目录或字段相似性直接合并/删除。 |
