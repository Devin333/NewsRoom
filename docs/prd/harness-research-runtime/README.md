...
| 26 | [26-unified-budget-governance.md](26-unified-budget-governance.md) | `READY_FOR_OPENSPEC` | `NOT_STARTED` | 阶段 20 的 `WF-002/U10`；现有 `framework/llm/budget`、Agent/Workflow 重复 `GlobalBudget*`；阶段 22 retry-credit 边界；阶段 23 cache 与阶段 24 context 接口 | 将跨调用累计的 LLM calls/tokens/cost 收敛到 `framework/governance/budget`，建立显式 scope、原子 reservation、exactly-once settlement、并发 admission 和 durable restore/replay；per-call、context、loop/stall/output、retry、tool/wall-time 与 diagnostics 继续由各自领域拥有。 |
