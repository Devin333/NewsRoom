...
| 20 | [20-framework-boundary-and-duplication-convergence.md](20-framework-boundary-and-duplication-convergence.md) | `READY_FOR_OPENSPEC` | `IN_PROGRESS` | 2026-07-18 框架层与重复实现专项审查；首个切片 `harness-deterministic-gate-enforcement` 已实现（`017a227e`）并归档；复用 `research-runtime-production-composition`、`framework-runtime-safety-hardening`；消费但不修改阶段 19 event contract | Harness VERIFY authority 已完成首个切片；后续继续收敛默认 Research composition、Tool approval/policy、Source/Quality/Workflow contract ownership 与可证删除的 legacy。 |
