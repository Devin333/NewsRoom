...
| 20 | [20-framework-boundary-and-duplication-convergence.md](20-framework-boundary-and-duplication-convergence.md) | `READY_FOR_OPENSPEC` | `IN_PROGRESS` | 2026-07-18 框架层与重复实现专项审查；Harness 切片已实现并归档；Source URL/rate/retry/error 的 12 个直接前序 changes 已同步 canonical specs；复用 `research-runtime-production-composition`、`framework-runtime-safety-hardening` | 下一切片为 `source-policy-contract-convergence`；随后继续收敛默认 Research composition、Tool approval/policy、Quality/Workflow contracts 与可证删除的 legacy。 |
