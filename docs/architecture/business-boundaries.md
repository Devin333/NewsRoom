...
source item、raw content 和 parsed-items index 的具体写入职责由 `_SourceItemArtifactWriter` 承载；`SourceArtifactWriter` 只负责编排 item/fetch/error 写入结果、维护 fetch request/result refs，并生成 source artifact index。新增 source artifact 类型时，应优先新增对应 input view 或小型 writer，不要把 payload 解析和文件写入细节继续堆回 `SourceArtifactWriter.write_source_artifacts()`。
