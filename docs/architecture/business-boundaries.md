...
- 当前闭环雏形允许 writer 根据 verifier feedback 做一次 source-bounded rewrite，也允许 analyst evidence gap 触发一次 planner/source recollect planning route，并已有正式 profile 与 execution plan application service 承载 recollect planning 输入。editor 产出的 rewrite / edited draft 仍由 finalization 消费。真正的 source fetch 执行应由后续应用层 executor 消费 `DailySourceRecollectionExecutionPlan` 后显式扩展。
