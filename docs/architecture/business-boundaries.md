...
`DailyOutputProjectionReadPolicy` 用于显式标注 projection 的读取策略：persistence、board attachment 和 run inspection 当前仍使用 `NAMESPACED_WITH_LEGACY_FALLBACK` 保持旧运行结果兼容，但 legacy fallback 不得再隐式散落在 consumer 中；下一轮下线旧 key 时，应逐个把剩余专用 projection 切到 `NAMESPACED_ONLY` 并补对应调用方测试。
