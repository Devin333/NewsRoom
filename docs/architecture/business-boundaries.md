...
`load_source_definitions()` 这类 definitions-only 入口默认也必须复用同一套 source definition validation；只有明确的 parse-only 调用才能显式传入 `validate=False`，避免调用方绕过 fixture URL、URL 凭证和 secret-like metadata 校验。
...
registered connector 返回的外部 fetch result 对象必须先投影为 source fetch result view，再构造正式 `SourceFetchResult`；adapter 的 protocol/sync 分支不应散落维护字段级 `hasattr` / `getattr` coercion。
...
`normalize_source_errors()` 是 `source_errors` 的唯一结构归一化入口：它拒绝顶层 mapping、字符串、bytes 和非结构化条目，只接受 `SourceError` 或完整序列化 mapping 序列。coverage、fallback、error policy、report writer、source recollection 和 artifact input 必须先归一化再消费正式字段，不能在各自模块里重新做 dict/object 混合读取。
...
source item artifact 输入由同一模块的 `SourceItemArtifactInput` 投影。raw item 的 `source_id`、`source_item_id`、raw content、raw/parse artifact ref 和 legacy mapping fallback 都属于 input view 的职责；`SourceArtifactWriter` 只消费该 view 的正式字段来写 raw content、source item 和 parsed-items index，不再在发布编排主体里通过 `_raw_content()` / `_existing_artifact_ref()`、`dict.get("source_item_id")` 或 artifact ref metadata 猜测 raw item 结构。
...
`output_projection` 不直接导入或遍历全量 `DAILY_BUFFER_ALIASES`，历史 alias 投影只能基于调用方显式传入的 key 范围解析。
...
artifact publisher 也进入命名空间优先输出读取阶段：发布器只能消费专用 artifact 投影视图；artifact-facing projection 由 `daily_intelligence.output_projection` 的 `project_daily_output_for_*_artifacts()` 承载，统一读取 `sources.*`、`evidence.*`、`quality.*`、`report.*`、`agent.feedback.*` 和 `agent.<label>.loop.*`。泛用 `daily_output_value()` / `daily_output_contains()` 只保留在 `output_projection.py` 内部作为兼容边界，legacy key 只作为这些 projection 内部的兼容兜底；发布器不得直接导入 `DAILY_BUFFER_ALIASES` 或重新维护 dotted/legacy 分支。对外 artifact key、manifest key 和文件路径继续保持现有稳定命名，例如 `quality_result.json`、`source_recollection/execution_report.json` 和 `agentic/agent_feedback_summary.json`。
quality artifact 和 manifest 字段由 `project_daily_output_for_quality_artifacts()` 投影后消费；evidence artifact 字段由 `project_daily_output_for_evidence_artifacts()` 投影后消费；source diagnostics artifact、manifest 和 source artifact index 输入由 `project_daily_output_for_source_diagnostic_artifacts()` 投影后消费；source recollection manifest 字段由 `project_daily_output_for_source_recollection_artifacts()` 投影后消费；agentic artifact 字段由 `project_daily_output_for_agentic_artifacts()` 投影后消费；report artifact 字段由 `project_daily_output_for_report_artifacts()` 投影后消费。legacy fallback 只保留在这些 artifact-facing projection 内，`quality_artifact_projection.py`、`evidence_artifact_projection.py`、`source_diagnostic_artifact_projection.py`、`source_recollection_artifact_projection.py` 和 `agentic_artifact_projection.py` 不直接调用泛用 output accessor，publisher 的 quality/report/source artifact 分支也不得重新维护 dotted/legacy 分支。
...
- `project_daily_output_for_interface_metadata()`：只为接口展示投影 agent loop metrics 等元数据；该 projection 已切到 `NAMESPACED_ONLY`，不再从 legacy-only `agent_loop_metrics` 构造接口指标。
...
- `project_daily_output_for_legacy_consumers()`：仅作为历史兼容 helper 保留；调用方必须显式传入 key 范围。新增服务消费面必须优先定义专用 projection，避免 consumer 直接理解 daily alias 表或接收整份 runtime output。
...
agentic writer 输出归一化阶段的 evidence lookup 由 `grounded_writer_evidence_view.py` 投影：`grounded_writer.py` 只消费按 evidence id 选择后的 `GroundedWriterEvidenceSelection` 来组装 grounded sections，不直接扫描 bundle items 或重复 source URL 聚合规则。
...
- `DailySourceRecollectionExecutor` 的 task/item 运行期计数由 `SourceRecollectionTaskItemTracker` 承载；raw item 的 `source_item_id` 和 metadata 先投影为 source recollection item view，再由 tracker/projector 消费。`DailySourceRecollectionArtifactProjector` 只负责把 plan/task context 投影到 fetch request、raw item 和 skipped source metadata，供 artifact 与历史兼容使用，不在 executor 主流程里散落 `getattr` / dict 分支。
...
- `ProductizedSignalPreparationService` 和 `ProductizedImprovementWorkflowService` 负责 policy experiment application 输出协调，并通过 `policy_experiment_application_context` 正式承载 `run_id`、`board_type`、applied/skipped policy experiments、proposal ids 和 measurement plan；该正式 context 使用 `PolicyExperimentApplicationContext.to_application_dict()`，不包含 legacy override 字段。旧 `improvement_context` 只作为 prepare step / run state 的兼容别名保留。productized workflow 正式输出 `applied_policy_experiments` / `skipped_policy_experiments`，不再输出 `applied_overrides`。productized artifacts 以 `policy_experiment_application_context.json`、`applied_policy_experiments.json` 和 `skipped_policy_experiments.json` 作为正式策略实验产物，`applied_overrides.json` 由 artifact publisher 从 `applied_policy_experiments` 派生，只保留为旧消费者兼容视图。
- `business.boards.productized.artifact_payloads` 是 productized artifact payload projection 边界，负责把正式 output 投影为 artifact payload，并集中承载 `applied_overrides.json` 这类历史兼容产物派生规则；`BoardArtifactPublisher` 只负责编排写文件和 manifest，不直接维护 override 兼容分支。
...
`business.foundation` 与 `business.foundation.feedback` 根出口只导出正式 policy experiment 类型，不再导出 `BoardImprovementContext`、`ImprovementOverride`、`LegacyPolicyExperimentPatch` 或 `SUPPORTED_OVERRIDE_TYPES`；旧名称如确需兼容读取，必须从 `business.foundation.feedback.override_policy` 显式导入。
...
`BoardImprovementContext` 只作为 `PolicyExperimentApplicationContext` 的历史兼容别名保留；`business/boards` 新代码和测试必须使用正式类型名。
