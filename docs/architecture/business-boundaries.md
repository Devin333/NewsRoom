...
social-media evidence gate 的输入投影由 `source_gate_evidence.py` 承载：evidence item 的 `source_type`、`category` 和 `source_url` 先归一化为 `SourceGateEvidenceItemView`，bundle 计数通过 `SourceGateEvidenceBundleView.item_count` 暴露给 bypass assessment。`source_gate_policy.py` 和 `quality_gate_policy.py` 只消费该 view 做规则判定与事件 metadata 构建；metadata 读取仅保留在该投影边界内，不能散落进 quality gate policy、usecase 或 workflow step。
