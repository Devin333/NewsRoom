...
- `SourceConnectorRuntimeOptions`：承载 daily source connector 运行参数，集中投影 connector-specific legacy metadata 与 request topic，包括 arXiv query、GitHub repository/query/collection mode、Hacker News story list、Reddit listing 选择和 community tag/site；source collection / recollection / dispatcher / fetch request 只消费该正式 view。
