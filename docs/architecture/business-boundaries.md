...
- analyst source recollection 输入必须写入 `DailyAgentFeedbackEvent.evidence_gaps`、`source_recollection_requests` 和 `missing_information` 正式字段；旧 `event.metadata` 只作为历史 payload 投影入口。
