...
quality manifest 字段由 `project_daily_output_for_quality_artifacts()` 投影后消费；legacy fallback 只保留在该 artifact-facing projection 内，`quality_artifact_projection.py` 不直接调用泛用 output accessor。
