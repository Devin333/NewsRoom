...
- source/model 配置、workflow buffer、business layer boundary 已有基础 guard；`finalize_report` 与 source/evidence 主链路已先行改为命名空间优先声明/消费，后续应继续把 dotted key 迁移从兼容双写推进到 agent loop 和 recollect step 的正式消费。
