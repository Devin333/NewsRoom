...
5. source connector metadata 读取已由架构测试固定在 `SourceConnectorRuntimeOptions` / metadata view 边界，并通过 `source_connector_metadata_fallback_plan.py` 固定历史 fallback key 下线顺序；其中 GitHub `metadata.mode` fallback 已下线，GitHub source 配置改为正式 `metadata.github_mode`。
...
- [x] 为 source connector metadata fallback 制定历史 key 下线顺序。
- [x] 按 `source_connector_metadata_fallback_plan.py` 执行 source connector metadata fallback key 第一阶段下线：GitHub `mode` -> `github_mode`。
- [ ] 继续按 `source_connector_metadata_fallback_plan.py` 执行 Reddit `time`、StackOverflow `tag`、manual `records` 的分阶段下线。
