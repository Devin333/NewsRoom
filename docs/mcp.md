...
into workflow executors, storage clients, or Harness internals directly.
...
`news.run.cancel` requests workflow run cancellation through
...
arguments for clients, but they do not route workflows, decide quality gates, or
write memory. Harness and application services remain responsible for runtime
decisions.
