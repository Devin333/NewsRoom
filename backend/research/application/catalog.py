...
class ResearchPaperCatalogService:
    ...
    def get_code(
        ...
        *,
        ...
        for relation in entry.relations:
            ...
            profiles.append(dict(profile) if isinstance(profile, Mapping) else {"repositoryId": relation.target_id, "status": relation.status})
        return profiles
    ...
    def get_benchmarks(
        ...
        *,
        ...
        return {"paperId": paper_id, "relations": relations, "scores": [score.model_dump(mode="json", exclude_none=True) for score in scores]}
    ...
    def verify_score(
        ...
        *,
        ...
        if benchmark is not None and score.benchmark_id != benchmark.benchmark_id:
            reasons.append("benchmark_mismatch")
        if dataset is not None:
            if score.dataset_id != dataset.dataset_id:
                reasons.append("dataset_mismatch")
            if score.metadata.get("dataset_version") and dataset.version and score.metadata["dataset_version"] != dataset.version:
                reasons.append("dataset_version_mismatch")
        if metric is not None:
            if score.metric_id != metric.metric_id:
                reasons.append("metric_mismatch")
            if score.direction and score.direction != metric.direction:
                reasons.append("metric_direction_mismatch")
            if score.unit and metric.unit and score.unit != metric.unit:
                reasons.append("metric_unit_mismatch")
        if expected_protocol is not None and (score.evaluation_protocol or "") != expected_protocol:
    ...
    def compare_scores(
        ...
        *,
        ...
        selected = [score for score in scores if (benchmark_id is None or score.benchmark_id == benchmark_id) and (metric_id is None or score.metric_id == metric_id)]
        ...
        for score in selected:
            ...
            key = metric_compatibility_key(
                ...
                dataset_version=score.metadata.get("dataset_version"),
    ...
    def list_leaderboards(
        ...
        *,
        ...
        for score in scores:
            if benchmark_id is not None and score.benchmark_id != benchmark_id:
                continue
            if metric_id is not None and score.metric_id != metric_id:
                continue
            key = metric_compatibility_key(
                dataset_id=score.dataset_id,
                dataset_version=score.metadata.get("dataset_version"),
        ...
        return [
            self.compare_scores(group, benchmark_id=key[0], metric_id=key[1]).model_dump(mode="json", exclude_none=True)
        ...
        *,
        ...
        for relation_type, target_type, target_id in candidates:
            ...
            if target_type == "code_repository":
                profile = metadata.get("code_profile")
        ...
        for score in score_candidates or []:
            ...
            relations.append(ResearchPaperRelation(
                ...
                observed_at=datetime.now(UTC),
                metadata={"actor_scope": dict(actor_scope or {})},
            ))
        return relations
            ...
            result.append(ResearchScore(
                ...
                metadata={
                    **dict(item),
                    ...
                    "dataset_version": _candidate_text(item, "dataset_version", "datasetVersion") or metadata.get("dataset_version"),
                },
            ))
        return result
...
class InMemoryResearchCatalogRepository(
    ...
    def save(self, value: Any) -> None:
        with self._lock:
            if isinstance(value, ResearchPaperCatalogEntry):
                ...
                self._snapshots[_scoped_key(value.snapshot_id, _snapshot_scope(value))] = value
    ...
    def get_identity(self, paper_id: str, *, actor_scope: Mapping[str, str] | None = None) -> ResearchPaperIdentity | None:
        with self._lock:
            return self._identities.get(_scoped_key(paper_id, actor_scope)) or (
    ...
    def get_paper(self, paper_id: str, *, actor_scope: Mapping[str, str] | None = None) -> ResearchPaper | None:
        with self._lock:
            return self._papers.get(_scoped_key(paper_id, actor_scope)) or (
    ...
    def get_snapshot(self, snapshot_id: str, *, actor_scope: Mapping[str, str] | None = None) -> ResearchSourceSnapshot | None:
        with self._lock:
            return self._snapshots.get(_scoped_key(snapshot_id, actor_scope)) or (
    ...
    def get_document(self, paper_id: str, *, actor_scope: Mapping[str, str] | None = None) -> ResearchDocument | None:
        with self._lock:
            return self._documents.get(_scoped_key(paper_id, actor_scope)) or (
    ...
    def get_evidence(self, paper_id: str, *, actor_scope: Mapping[str, str] | None = None) -> ResearchEvidencePack | None:
        with self._lock:
            return self._evidence.get(_scoped_key(paper_id, actor_scope)) or (
    ...
    def find_by_external_id(self, external_id: str, *, actor_scope: Mapping[str, str] | None = None) -> ResearchPaperIdentity | None:
        needle = str(external_id).casefold()
        with self._lock:
            return next((identity for identity in self._identities.values() if actor_scope_matches(identity.metadata.get("actor_scope"), actor_scope) and needle in {str(value).casefold() for value in (identity.arxiv_id, identity.doi, identity.openreview_id, identity.canonical_url) if value}), None)
...
def _snapshot_scope(snapshot: ResearchSourceSnapshot) -> Mapping[str, Any]:
    return {**dict(snapshot.metadata), **dict(snapshot.lineage.metadata)}
...
def _merge_relations(existing: Iterable[ResearchPaperRelation], incoming: Iterable[ResearchPaperRelation]) -> list[ResearchPaperRelation]:
    ...
    for relation in incoming:
        ...
        if old is None or (old.status != "verified" and relation.status == "verified"):
