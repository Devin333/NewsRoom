...
def _normalize_source_error(value: Any, *, context: str) -> SourceError:
    ...
    return _source_error_from_object(value, context=context)
...
def _source_error_from_object(value: Any, *, context: str) -> SourceError:
    if not all(hasattr(value, field_name) for field_name in _REQUIRED_FIELDS):
        raise TypeError(
            f"{context} entries must be SourceError-compatible values "
