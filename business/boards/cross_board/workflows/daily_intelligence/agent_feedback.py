...
def collect_agent_feedback(buffer: StepScopedDataBufferView) -> dict[str, Any]:
    ...
    routing_service = DailyAgentFeedbackRoutingService()
    loop_state = routing_service.next_loop_state(
        previous_loop_state=_optional_buffer_dict(buffer, "agent_feedback_loop_state"),
        summary=summary,
