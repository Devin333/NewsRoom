...
class CrossBoardPathFinder:
    def find_paths(self, graph: CrossBoardGraph, request: CrossBoardPathSearchRequest | None = None) -> CrossBoardPathSearchResult:
        ...
        for technology_ref in technology_refs:
            ...
            path_score = _path_score(chain, missing)
...
def _path_score(chain, missing: list[str]) -> float:
    completeness = (len(ORDERED_STAGE_TYPES) - len(missing)) / len(ORDERED_STAGE_TYPES)
    board_support = min(1.0, chain.board_support_count / len(ORDERED_STAGE_TYPES))
    confidence = chain.average_relation_confidence
    evidence_diversity = max(0.0, min(1.0, (chain.evidence_count - chain.duplicate_evidence_count) / len(ORDERED_STAGE_TYPES)))
    contradiction_penalty = 0.25 if chain.contradictory_evidence_count else 0.0
    score = 0.35 * completeness + 0.25 * board_support + 0.25 * confidence + 0.15 * evidence_diversity - contradiction_penalty
