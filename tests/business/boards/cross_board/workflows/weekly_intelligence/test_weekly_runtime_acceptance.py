...
def test_weekly_runtime_acceptance_uses_local_fixture_reports(tmp_path) -> None:
    end = datetime(2026, 5, 22, 23, 59, 59, tzinfo=UTC)
    _write_daily_report_fixture(tmp_path, "weekly-runtime-daily-1", finished_at=end - timedelta(days=1))
    _write_daily_report_fixture(tmp_path, "weekly-runtime-daily-2", finished_at=end - timedelta(days=2))
    ...
    result = WeeklyIntelligenceRunner(artifact_root=tmp_path).run(
        ...
        run_id="weekly-runtime-acceptance",
    )

    assert result.status == WorkflowStatus.SUCCEEDED
    assert result.output["final_report"]
    assert result.output["report_markdown"].strip()
    assert result.output["weekly_metrics"]["source_report_count"] >= 1
    ...
    for file_name in (
        ...
        assert artifact.exists(), file_name
        assert json.loads(artifact.read_text(encoding="utf-8"))


def _write_daily_report_fixture(root: Path, run_id: str, *, finished_at: datetime) -> None:
    run_dir = root / run_id
        ...
        "sections": [
            {
                ...
                "content": "Agent Memory, OpenAI, LangChain, MCP, and quality evidence recurred in productized output.",
                "sources": ["https://example.com/agent-memory"],
            }
        ],
        "metadata": {
            "topic": "Agent Memory",
            "cross_board_summary": "Offline runtime acceptance fixture.",
        },
    }
    manifest = {
        "run_id": run_id,
