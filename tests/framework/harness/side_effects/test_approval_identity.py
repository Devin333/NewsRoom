...
def test_terminal_approval_evidence_matches_without_physical_identity() -> None:
    request = _request(
        node_id=None,
        node_instance_id=None,
        activity_id=None,
    ...
    evidence = HarnessSideEffectApprovalEvidence(
        approval_ref=checksum_for("terminal-approval"),
        run_id=request.run_id,
        graph_id=request.graph_id,
        graph_version=request.graph_version,
        graph_ref=request.graph_ref,
    ...
    resolved = InMemoryHarnessSideEffectApprovalResolver((evidence,)).resolve(
        request,
