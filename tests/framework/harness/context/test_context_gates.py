...
def test_context_gates_accept_valid_assembled_envelope() -> None:
    envelope = ContextAssembler().assemble({"source_refs": ("source://paper#section=abstract",)})
...
def test_stable_prefix_gate_rejects_dynamic_tool_result() -> None:
    ...
    bad = replace(envelope, segments=(bad_segment, *envelope.segments[1:]))
...
def test_privacy_gate_rejects_private_memory_without_consent() -> None:
    ...
    bad = replace(envelope, segments=(*envelope.segments[:4], private_segment, envelope.segments[5]))
