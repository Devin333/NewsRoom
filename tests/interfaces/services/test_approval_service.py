...
def test_approval_service_has_no_resume_context_or_state_patch_surface() -> None:
    service = ApprovalApplicationService(store=InMemoryApprovalStore())

    assert not hasattr(service, "build_resume_context")
    assert not hasattr(service, "resume_context")
    assert "buffer_updates" not in service.__class__.__dict__
    assert "resume_metadata" not in service.__class__.__dict__
