...
export function RankedProjectCard({
  project,
  rank,
  rankReason,
}: {
  ...
  return (
    <article className="flex h-full flex-col rounded-md border border-[#d8dee7] bg-white p-4 shadow-sm dark:border-border dark:bg-card">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-[#0f172a] text-sm font-bold text-white dark:bg-primary">
              #{rank}
            </span>
            <p className="truncate font-mono text-xs text-[#667085] dark:text-muted-foreground">{project.github_url ?? project.canonical_url ?? project.id}</p>
          </div>
      ...
      {rankReason && (
        <p className="mt-2 line-clamp-2 text-xs text-[#667085] dark:text-muted-foreground">{rankReason}</p>
      )}

      <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#4b5563] dark:text-muted-foreground">
        {project.description ?? project.tagline ?? "No description was available in the real Project Radar artifact."}
      </p>

      <div className="mt-3">
        <div className="flex items-center justify-between text-xs mb-1">
          <span className="text-[#667085] dark:text-muted-foreground">Heat Score</span>
          <span className="font-semibold text-[#202124] dark:text-foreground">{formatScore(score ?? undefined)}</span>
        </div>
        <div className="h-2 rounded-full bg-[#e5e7eb] dark:bg-border overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-[#ff6b6b] to-[#ffa500]"
            style={{ width: `${Math.min(score ?? 0, 1) * 100}%` }}
          />
        </div>
      </div>
