...
export function QualityLineageGraph({ claims }: { claims: StudioClaimEvidence[] }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Evidence Lineage</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {featuredClaims.length ? (
          <div className="space-y-3">
            {featuredClaims.map((claim) => (
              <div key={claim.claimId} className="grid gap-2 rounded-md border border-border bg-card p-3 text-sm md:grid-cols-5">
                <LineageCell value={claim.sourceRefs[0]?.url ?? claim.sourceRefs[0]?.title ?? claim.sourceRefs[0]?.sourceId ?? "No source"} />
                <LineageCell value={claim.evidenceRefs[0]?.evidenceId ?? claim.evidenceRefs[0]?.summary ?? claim.evidenceRefs[0]?.quote ?? "No evidence"} />
                <LineageCell value={claim.claimText} strong />
                <LineageCell value={claim.reportSection ?? "No section"} />
          ...
          <p className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground">No claim lineage rows were generated.</p>
        )}
      </CardContent>
    </Card>
  )
