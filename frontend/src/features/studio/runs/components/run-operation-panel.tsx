    ...
    () => [
      {
        type: "cancel",
        label: "Cancel run",
        description: "Stop a pending or running workflow.",
        enabled: detail.operations.canCancel,
        icon: <Ban className="size-4" />
      },
      {
        type: "rerun-from-step",
        label: "Rerun from selected step",
        description: "Resume execution from the selected step.",
        enabled: detail.operations.canRerunFromStep && Boolean(selectedStep),
        icon: <RotateCcw className="size-4" />
      },
      {
        type: "skip-step",
        label: "Skip selected step",
        description: "Mark the selected step as skipped.",
        enabled: detail.operations.canSkipStep && Boolean(selectedStep),
        icon: <FastForward className="size-4" />
      },
      {
        type: "mark-blocked-resolved",
        label: "Mark blocked resolved",
        description: "Record that human intervention resolved the block.",
        enabled: detail.operations.canResolveBlocked,
        icon: <ShieldCheck className="size-4" />
      }
    ],
    [detail.operations, selectedStep]
  )
    ...
    <section className="space-y-4 rounded-lg border border-border bg-card p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Dangerous operations require confirmation and a reason.</p>
        </div>
        {detail.dataState === "fallback" ? <Badge tone="warning">Disabled for fallback</Badge> : null}
      </div>
      ...
      <form className="space-y-3" onSubmit={onSubmit}>
        {requiresStep ? (
          <div className="rounded-md border border-border bg-secondary/35 p-3">
            <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">Selected step</p>
            <p className="mt-1 font-mono text-xs text-foreground">{selectedStep?.id ?? "No step selected"}</p>
          </div>
        ) : null}
        <label className="grid gap-1 text-sm text-muted-foreground">
          Reason
          <Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Required reason for audit trail" />
        </label>
        <label className="grid gap-1 text-sm text-muted-foreground">
          Actor ID
          <Input value={actorId} onChange={(event) => setActorId(event.target.value)} placeholder="Optional operator id" />
        </label>
        <label className="flex items-start gap-2 rounded-md border border-warning/30 bg-warning/10 p-3 text-sm text-warning">
          ...
          I confirm this operation should be sent to the Run Runtime API.
        </label>

        {!active.enabled ? (
          <p className="flex items-center gap-2 text-sm text-muted-foreground">
            <AlertTriangle className="size-4" />
            This operation is not available for the current run state.
          </p>
        ) : null}
        ...
        {result ? (
          <div className={`rounded-md border p-3 text-sm ${result.ok ? "border-success/30 bg-success/10 text-success" : "border-danger/30 bg-danger/10 text-danger"}`}>
            <p>{result.ok ? result.message ?? "Operation accepted." : result.message ?? "Operation failed."}</p>
        ...
        <Button type="submit" variant={operation === "cancel" ? "destructive" : "default"} disabled={!canSubmit}>
          {isPending ? "Submitting..." : active.label}
        </Button>
      </form>
    </section>
  )
