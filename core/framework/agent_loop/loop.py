...
class AgentLoop:
    ...
    def run(
        ...
        for iteration in range(1, agent.loop_policy.max_iterations + 1):
            ...
            action = AgentAction(action_type=action.action_type, output=normalized_output)
...
def _normalize_agent_output(
    *,
    ...
    if agent.agent_id != "daily.writer" or _request_profile(inputs) != "agentic-live":
        return output
    report_draft = output.get(agent.output_key)
    ...
    normalized_output[agent.output_key] = _normalized_writer_report_draft(
        report_draft,
